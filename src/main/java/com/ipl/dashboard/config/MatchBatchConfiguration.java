...
public class MatchBatchConfiguration {
	...
	private final static String iplMatchFileNameWithPath = "match-data.csv";
	
	private final static String[] CSV_FILE_FIELDS_NAMES = {"id", "city", "date", "player_of_match", "venue", "neutral_venue", 
			"team1", "team2", "toss_winner", "toss_decision", "winner", "result", "result_margin", "eliminator", "method",
			"umpire1", "umpire2"}; 
	@Bean
	public FlatFileItemReader<MatchInput> reader() {
		return new FlatFileItemReaderBuilder<MatchInput>().name("MatchItemReader")
				.resource(new ClassPathResource(iplMatchFileNameWithPath)).delimited()
				.names(CSV_FILE_FIELDS_NAMES)
	...
	public JdbcBatchItemWriter<Match> writer(DataSource dataSource) {
		return new JdbcBatchItemWriterBuilder<Match>()
				...
				.sql("INSERT INTO match (id, city, date, player_of_match, venue, team1, team2, toss_winner, toss_decision, "
						+ "match_winner,result, result_margin, umpire1, umpire2 ) "
